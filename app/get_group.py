...
class Group:
    ...
    def response(self, url: str) -> BeautifulSoup:
        response = requests.get(url, headers={"Host": self.host}, cookies=self.cookies)
    ...
    def CollectPost(self, soup: BeautifulSoup, count: int) -> list:
        ...
        for comment in re.findall(
            r'href="(https:\/\/mbasic\.facebook\.com\/groups\/\d+\/permalink\/\d+\/.*?)">.*?Komentar</a>',
    ...
    def GetInfoGroup(self, url: str, count: int) -> dict:
